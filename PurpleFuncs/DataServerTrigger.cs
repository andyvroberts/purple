...
{
    public class DataServerTrigger
    {
        /// <summary>
            ...
            FunctionContext context)
        {
            ...
            _logger.LogInformation("Search Filter = {queryFilter}", queryFilter);

            // setup the deferred query.  Azure Table Storage max entities returned per call = 1,000 but we use 50.
        ...
        private static async Task<List<PriceResult>> TableQueryPagination(ILogger _log, int maxPageCount, AsyncPageable<PriceData> queryResults)
        {
            ...
            await foreach (Page<PriceData> pricePage in queryResults.AsPages())
            {
                ...
                {
                    _log.LogWarning("Maximum pages reached {pageCounter}.", pageCounter);
                    break;
                }
            }
            return returnData;
